...
void
_exit_with_error (char *msg, int code)
{
  perror (msg);
  exit (code);
}
...
void
send_failure_message (int socket)
{
...
void
send_success_message (int socket)
{
  char *out_buffer = (char *)malloc (BUFFER_SIZE);
...
void
process_http_request (int server_socket)
{
  ...
  while (1)
    {
      ...
      incoming_socket
          = accept (server_socket, (struct sockaddr *)&incoming_addr,
                    &incoming_addr_len);
