...
void
base64_encode (char *src, int n, char *dest)
{
  ...
  for (int i = 0; i < n; i = i + 3)
    {
      ...
      if (i + 2 < n)
        {
          dest[j++]
              = base64[(src[i + 1] & 0x0F) << 2 | (src[i + 2] & 0xC0) >> 6];
        }
      else
        {
...
int
base64_decode (char *src, int n, char *dest)
{
  ...
  for (int i = 0; i < n; i = i + 4)
    {
      ...
      if ((unbase64[src[i + 0]] == -1) || (unbase64[src[i + 1]] == -1)
          || (unbase64[src[i + 2]] == -1) || (unbase64[src[i + 3]] == -1))
        {
          return -1;
        }
