...
void print_all_addr(struct addrinfo *resolved_host)
{
	...
	for (struct addrinfo *cur_addr = resolved_host; cur_addr != NULL;
		...
		if (cur_addr->ai_family == AF_INET) { // IPv4
			...
			struct sockaddr_in6 *ipv6 =
				(struct sockaddr_in6 *)cur_addr->ai_addr;
			addr = &(ipv6->sin6_addr);
			ipver = "IPv6";
		}
...
*/
int parse_proxy_params(char *proxy_params, char **proxy_host, char **proxy_port,
		       char **proxy_user, char **proxy_passwd)
{
	...
	if (auth_part) {
		...
		**proxy_passwd = '\0'; // Delimits user
		...
		current_pos =
			++auth_part; // Increment auth_part because it points to '@'
	}
...
int http_get(int sockfd, char *host, char *path, char *proxy_host,
	     char *proxy_user, char *proxy_passwd)
{
	...
	if (proxy_user) {
		...
		fprintf(stdout, "%s", message_buffer);
		if (send(sockfd, message_buffer, strlen(message_buffer), 0) ==
		    -1) {
			return -1;
		}
	}
...
int main(int argc, char **argv)
{
	...
	if (argc < 2) {
		fprintf(stderr,
			"Usage : http_client -p <optional proxy params> <url>\n");
		exit(1);
	}

	char *host, *path, *proxy_host, *proxy_user, *proxy_passwd,
		*requested_url, *proxy_port;

	host = NULL;
	path = NULL;
	proxy_host = proxy_user = proxy_passwd = NULL;
	if (strcmp("-p", argv[1]) == 0) {
		if (parse_proxy_params(argv[2], &proxy_host, &proxy_port,
				       &proxy_user, &proxy_passwd) == -1) {
	...
	if (proxy_host) {
		getaddrinfo(proxy_host, proxy_port ? proxy_port : default_port,
			    &hints, &resolved_host);
	} else {
	...
	for (struct addrinfo *cur_addr = resolved_host;
	     cur_addr != NULL && !connected; cur_addr = cur_addr->ai_next) {
		sockfd = socket(cur_addr->ai_family, cur_addr->ai_socktype,
				cur_addr->ai_protocol);

		if (sockfd == -1) {
			fprintf(stderr,
				"Warning : Could not create socket - %s\n",
				strerror(errno));
			continue;
		}

		if (connect(sockfd, cur_addr->ai_addr, cur_addr->ai_addrlen) ==
		    -1) {
			fprintf(stderr,
				"Warning : Could not connect to the host - %s\n",
				strerror(errno));
	...
	if (http_get(sockfd, host, path, proxy_host, proxy_user,
		     proxy_passwd) == -1) {
