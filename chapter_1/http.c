...
void
print_all_addr (struct addrinfo *resolved_host)
{
...
*/
int
parse_proxy_params (char *proxy_params, char **proxy_host, char **proxy_port,
...
int
http_get (int sockfd, char *host, char *path, char *proxy_host,
          char *proxy_user, char *proxy_passwd)
{
...
main (int argc, char **argv)
{
  ...
  if (strcmp ("-p", argv[1]) == 0)
    {
      if (parse_proxy_params (argv[2], &proxy_host, &proxy_port, &proxy_user,
                              &proxy_passwd)
          == -1)
        {
  ...
  if (proxy_host)
    {
      getaddrinfo (proxy_host, proxy_port ? proxy_port : default_port, &hints,
                   &resolved_host);
    }
  else
    {
  ...
  if (http_get (sockfd, host, path, proxy_host, proxy_user, proxy_passwd)
      == -1)
    {
