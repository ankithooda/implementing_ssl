...
rol (unsigned char *target)
{
  ...
  target[3]
      = (((target[3] << 1) | ((target[4] & 0x80) >> 7)) & ~0x10) | left_carry;
...
generate_key_schedule (const unsigned char *key)
{
  ...
  for (int round = 1; round <= 16; round++)
    {
      ...
      permute ((unsigned char *)(keys + ((round - 1) * SUBKEY_SIZE)), pc1_key,
               key_permutation_2, SUBKEY_SIZE);
    }
  return keys;
}

static void
des_block_operate (const unsigned char plaintext[DES_BLOCK_SIZE],
                   unsigned char ciphertext[DES_BLOCK_SIZE],
                   const unsigned char key[DES_KEY_SIZE], op_type operation)
{

  unsigned char
      ip_block[DES_BLOCK_SIZE]; // Holds the plaintext that is being encrypted.
  unsigned char pc1_key[PC1_KEY_SIZE]; // Initial permuted key (56-bits)
  unsigned char
  ...
  unsigned char
      post_substitution_pbox[DES_BLOCK_SIZE
                             / 2]; // Target of the permutation after s-box
