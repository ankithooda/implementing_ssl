...
static void xor
	(unsigned char *target, const unsigned char *src, int len) {
		while (len--) {
			*target++ ^= *src++;
		}
	}
	...
	static void permute(unsigned char *target, const unsigned char *src,
			    const int *permute_table, const int len)
{
...
static const int p_table[] = { 16, 7, 20, 21, 29, 12, 28, 17, 1,  15, 23,
			       26, 5, 18, 31, 10, 2,  8,  24, 14, 32, 27,
			       3,  9, 19, 13, 30, 6,  22, 11, 4,  25 };

// This is the original specification s-box not the simplified s-box that
// Joshua Davies has shown in the book. There are a total of 8 s-boxes
// consisting of 64 4-bit integers
...
static void rol(unsigned char *target)
{
	...
	target[3] = (((target[3] << 1) | ((target[4] & 0x80) >> 7)) & ~0x10) |
		    left_carry;
...
static void ror(unsigned char *target)
{
	...
	target[3] = ((target[3] >> 1) & 0x70) | ((target[2] << 7) & 0x80) |
		    (target[3] & 0x0F);
	...
	target[3] = ((target[3] >> 1) & 0x07) | ((right_carry << 3) & 0x08) |
		    (target[3] & 0xF0);
}

// Returns a memory buffer consisting of
// 16 keys of SUBKEY_SIZE
static unsigned char *generate_key_schedule(const unsigned char *key)
{
	size_t buffer_size = sizeof(unsigned char) * SUBKEY_SIZE * 16;

	unsigned char *keys;

	keys = (unsigned char *)malloc(buffer_size);
	memset((void *)keys, 0, buffer_size);

	unsigned char pc1_key[PC1_KEY_SIZE]; // Initial permuted key (56-bits)

	// Initial Key permutation - Permutation Choice 1
	...
	for (int round = 1; round <= 16; round++) {
		...
		// Permute the pc1_key to create subkey for this round - Permutation
		// Choice 2

		permute((unsigned char *)(keys + ((round - 1) * SUBKEY_SIZE)),
			pc1_key, key_permutation_2, SUBKEY_SIZE);
	}
	return keys;
}
			      ...
			      op_type operation)
{
	unsigned char ip_block
		[DES_BLOCK_SIZE]; // Holds the plaintext that is being encrypted.
	unsigned char pc1_key[PC1_KEY_SIZE]; // Initial permuted key (56-bits)
	unsigned char
		expanded_block[EXPANSION_BLOCK_SIZE]; // Holds the expanded 48-bits
	unsigned char subkey[SUBKEY_SIZE];
	unsigned char substituted_block[DES_BLOCK_SIZE / 2];
	unsigned char post_substitution_pbox
		[DES_BLOCK_SIZE / 2]; // Target of the permutation after s-box
	...
	for (int round = 1; round <= 16; round++) {
		...
		permute(expanded_block, ip_block + 4, expansion_table,
		...
		printf("ORIG KEY ");
		print_hex_data(subkey, SUBKEY_SIZE);
		printf("KEY SCHD ");
		print_hex_data((unsigned char *)key_schedule +
				       ((round - 1) * SUBKEY_SIZE),
			       SUBKEY_SIZE);
		...
		unsigned char
			curr_byte; // Holds the 6-bits input required for the S-box
		int i, j;
		...
		// Remaining 2-bits of expanded_block[0] and first 4-bits of
		// expanded_block[1]
		curr_byte = ((expanded_block[0] << 4) & 0x30) |
			    ((expanded_block[1] >> 4) & 0x0F);
		i = (curr_byte & 0x01) | ((curr_byte >> 4) & 0x02);
		j = (curr_byte >> 1) & 0x0F;
		substituted_block[0] |= s_box[1][i][j] & 0x0F;

		// Remaining 4-bits of expanded_block[1] and first 2-bits of
		// expanded_block[2]
		curr_byte = ((expanded_block[1] << 2) & 0x3C) |
			    ((expanded_block[2] >> 6) & 0x03);
		i = (curr_byte & 0x01) | ((curr_byte >> 4) & 0x02);
		j = (curr_byte >> 1) & 0x0F;
		...
		// Remaining 2-bits of expanded_block[3] and first 4-bits of
		// expanded_block[4]
		curr_byte = ((expanded_block[3] << 4) & 0x30) |
			    ((expanded_block[4] >> 4) & 0x0F);
		i = (curr_byte & 0x01) | ((curr_byte >> 4) & 0x02);
		j = (curr_byte >> 1) & 0x0F;
		substituted_block[2] |= s_box[5][i][j] & 0x0F;

		// Remaining 4-bits of expanded_block[4] and first 2-bits of
		// expanded_block[5]
		curr_byte = ((expanded_block[4] << 2) & 0x3C) |
			    ((expanded_block[5] >> 6) & 0x03);
		i = (curr_byte & 0x01) | ((curr_byte >> 4) & 0x02);
		j = (curr_byte >> 1) & 0x0F;
		...
		permute(post_substitution_pbox, substituted_block, p_table,
			DES_BLOCK_SIZE / 2);
		...
		memcpy((void *)swap_buffer, (void *)ip_block,
		       DES_BLOCK_SIZE / 2);
